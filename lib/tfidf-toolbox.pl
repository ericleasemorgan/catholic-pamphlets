...
# December 8, 2018 - "enhanced" index to remove more OCR data
# March   30, 2019 - added round, ratio, and levenshtein; on a plane from Florence to Zurich
...
# compute Levenshtein distance as a ratio; results closer to 1 are closer to similarity
sub ratio {

	my $lhs = shift;
	my $rhs = shift;
	my $maxDist = length( $lhs );
	if ( length( $rhs ) > $maxDist ) { $maxDist = length( $rhs ) }
	if ( $maxDist == 0 ) { return 1 }
	return ( ( $maxDist - &levenshtein( $lhs, $rhs ) ) / $maxDist )
	
}


# compute Levenshtein "distance"; cool!
sub levenshtein {
